...
int main()
{
	double r1, r2;
	int n;
	while (scanf("%lf%lf%d", &r1, &r2, &n) != EOF){
		...
		for (int i = 1; i < n; i++)
			area2 += Addition(pos[i-1][0], pos[i-1][1], pos[i][0], pos[i][1], r2);
		area2 += Addition(pos[n-1][0], pos[n-1][1], pos[0][0], pos[0][1], r2);
		printf("%.2lf\n", abs(abs(area1) - abs(area2)));
	}
}
